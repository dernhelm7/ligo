...
match Location.unwrap d with
  ...
  | Declaration_constant {name ; binder ; attr={inline;no_mutation} ; expr} -> (
    ...
    let expr =
      ...
      type_expression' ~test ~protocol_version ?tv_opt:tv'_opt env expr in
...
and type_expression' ~raise ~test ~protocol_version : environment -> ?tv_opt:O.type_expression -> I.expression -> O.expression = fun e ?tv_opt ae ->
  ...
  | E_application {lamb; args} ->
      let lamb' = type_expression' ~raise ~test ~protocol_version e lamb in
      let args' = type_expression' ~raise ~test ~protocol_version e args in
  ...
  | E_matching {matchee;cases} -> (
    ...
    let case_exp = Pattern_matching.compile_matching ~raise ~err_loc:ae.location ~type_f:(type_expression' ~test ~protocol_version) ~body_t:(tv_opt) matcheevar eqs in
  ...
  | E_let_in {let_binder = {var ; ascr} ; rhs ; let_result; attr = { inline; no_mutation }} ->
    ...
    let rhs = type_expression' ~raise ~test ~protocol_version ?tv_opt:rhs_tv_opt e rhs in
    let binder = cast_var var in
    let e' = Environment.add_ez_declaration binder rhs e in
    let let_result = type_expression' ~raise ~test ~protocol_version e' let_result in
  ...
  | E_mod_in {module_binder; rhs; let_result} ->
    let env,rhs = type_module ~raise ~test ~protocol_version ~init_env:e rhs in
  ...
  | E_ascription {anno_expr; type_annotation} ->
    ...
    let expr' = type_expression' ~raise ~test ~protocol_version ~tv_opt:tv e anno_expr in
    let type_annotation =
  ...
  | E_module_accessor {module_name; element} ->
    ...
    in
    let element = type_expression' ~raise ~test ~protocol_version ?tv_opt module_env element in
