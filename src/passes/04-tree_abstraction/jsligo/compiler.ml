...
(* Switch case helpers *)
let get_region_switch_case = 
  function 
  | CST.Switch_case { kwd_case } -> kwd_case 
  | Switch_default_case { kwd_default } -> kwd_default

type case_statement_behaviour = 
    SwitchFallthrough of CST.statements option
  | SwitchBreak of CST.statements
  | SwitchReturn of CST.statements
type default_statement_behaviour = 
    DefaultBreak of CST.statements option
  | DefaultReturn of CST.statements
type switch_statement_behaviour = 
...
let case_behaviour (statements : CST.statements) = 
  ...
  match s with
  | CST.SBreak _  -> SwitchBreak statements
  | CST.SReturn _ -> SwitchReturn statements
  | _             ->
    let rec aux xs = 
      (match xs with
...
let default_behaviour (statements : CST.statements) = 
  let (s, statements') = statements in
  match s with
  | CST.SBreak _  -> DefaultBreak (Some statements)
  | CST.SReturn _ -> DefaultReturn statements
  | _             ->
    let rec aux xs = 
      (match xs with
...
let identify_case_behaviour (switch : CST.switch) =
   ...
   List.map cases ~f:(
     ...
     | CST.Switch_case { kwd_case; expr; statements = Some statements } -> Case (kwd_case, expr, (case_behaviour statements))
     | CST.Switch_case { kwd_case; expr; statements = None }            -> Case (kwd_case, expr, (SwitchFallthrough None))
     | CST.Switch_default_case { kwd_default ; statements = Some statements } -> Default (kwd_default, (default_behaviour statements))
     | CST.Switch_default_case { kwd_default ; statements = None }            -> Default (kwd_default, (DefaultBreak None)))
...
and compile_switch_cases ~raise (switch : CST.switch Region.reg) (rest : (CST.semi * CST.statement) list) : statement_result =
  ...
  | [] -> fun () -> e_unit ()
  | (_, statement)::rest -> 
    fun () -> statement_result_to_expression @@ compile_statements ~raise (statement, rest) rest in
  ...
  let aux result = function
    (_, hd) :: tl ->
      ...
      let block = compile_statement ~wrap:false ~raise wrapper rest in
  ...
  in
  let compile_statements ?(case_return=false) s = compile_statements ~case_return ~case:true s rest in
  ...
  let rec compile_cases 
    (cases : switch_statement_behaviour list) 
    ...
    | [] -> rest_of_the_code ()
    (* Single case *)
    | Case (kwd_case, expr, SwitchBreak statements)             ::[]
    | Case (kwd_case, expr, SwitchFallthrough (Some statements))::[] -> 
      ...
      statement_result_to_expression @@  aux ifexp rest
    | Case (_, expr, SwitchFallthrough None)::[] -> 
      let expr = rest_of_the_code () in
      use_switch_var switch_expr expr
    | Case (kwd_case, expr, SwitchReturn statements)     ::[] -> 
      let loc = Location.lift kwd_case in
      let case_expr = compile_expression ~raise expr in
      let condition = e_constant ~loc (Const C_EQ) [switch_expr; case_expr] in
      let condition = (match fallthrough_conditions with
      | Some fallthrough_conditions -> e_constant ~loc (Const C_OR) [condition; fallthrough_conditions]
      | None -> condition) in
      let then_clause = statement_result_to_expression @@ compile_statements ~case_return:true ~raise statements in
      let else_clause = rest_of_the_code () in
      e_cond ~loc condition then_clause else_clause
    (* Single default *)
    | Default (_, DefaultBreak (Some statements))::[] -> 
    ...
    | Default (_, DefaultReturn statements)     ::[] -> 
      ...
      use_switch_var switch_expr (e_cond cond then_clause else_clause)
    | Default (_, DefaultBreak None)      ::[] ->
      let expr = rest_of_the_code () in

      use_switch_var switch_expr expr
    (* Case - Case *)
    | Case (case1, expr1, SwitchFallthrough stmnts1)::
      Case (case2, expr2, SwitchFallthrough stmnts2)::cases -> 
      ...
      let rest = compile_cases cases (Some cond2) all_conditions in
      e_sequence combined_cases rest

    | Case (case1, expr1, SwitchBreak stmnts1)::
      Case (case2, expr2, SwitchFallthrough stmnts2)::cases -> 
      let loc1 = Location.lift case1 in
      let case_expr1 = compile_expression ~raise expr1 in
      let cond1 = e_constant ~loc:loc1 (Const C_EQ) [switch_expr; case_expr1] in
      ...
      | Some fallthrough -> e_constant ~loc:loc1 (Const C_OR) [cond1; fallthrough]
      | None -> cond1) in
      let code1 = statement_result_to_expression @@ compile_statements ~raise stmnts1 in
      ...
      let all_conditions = expr1::expr2::all_conditions in
      let rest = compile_cases cases (Some cond2) all_conditions in
      e_sequence part1 rest
    
    | Case (case1, expr1, SwitchReturn stmnts1)::
      Case (case2, expr2, SwitchBreak stmnts2) ::cases -> 
      let loc1 = Location.lift case1 in
      let case_expr1 = compile_expression ~raise expr1 in
      let cond1 = e_constant ~loc:loc1 (Const C_EQ) [switch_expr; case_expr1] in
      let cond1 = (match fallthrough_conditions with
      | Some fallthrough -> e_constant ~loc:loc1 (Const C_OR) [cond1; fallthrough]
      | None -> cond1) in
      let code1 = statement_result_to_expression @@ compile_statements ~case_return:true ~raise stmnts1 in

      let loc2 = Location.lift case2 in
      let case_expr2 = compile_expression ~raise expr2 in
      let cond2 = e_constant ~loc:loc2 (Const C_EQ) [switch_expr; case_expr2] in
      let code2 = compile_statements ~raise stmnts2 in
      let code2 = statement_result_to_expression code2 in
      let part2 = e_cond ~loc:loc2 cond2 code2 (e_unit ()) in

      let all_conditions = expr1::expr2::all_conditions in
      let rest = compile_cases cases None all_conditions in
      let rest = e_sequence part2 rest in

      e_cond ~loc:loc1 cond1 code1 rest
    ...
    | Case (case1, expr1, SwitchFallthrough stmnts1)::
      ...
      let code1 = Option.map stmnts1 ~f:(compile_statements ~raise) in
      let code1 = Option.map code1 ~f:statement_result_to_expression in
      let part1 = (match code1 with
      | Some code1 -> e_cond ~loc:loc1 cond1 code1 (e_unit ())
      | None -> e_unit ()) in
      ...
      let code1 = statement_result_to_expression @@ compile_statements ~raise stmnts2 in
    ...
    | Case (case1, expr1, SwitchBreak stmnts1)::
      ...
      let case_expr2 = compile_expression ~raise expr2 in
      let cond2 = e_constant ~loc:loc2 (Const C_EQ) [switch_expr; case_expr2] in
      let code2 = statement_result_to_expression @@ compile_statements ~raise stmnts2 in
      let part2 = e_cond ~loc:loc2 cond2 code2 (e_unit ()) in
      ...
      e_sequence combined_parts rest

    | Case (case1, expr1, SwitchFallthrough stmnts1)::
      Case (case2, expr2, SwitchReturn stmnts2)::cases -> 
      let loc1 = Location.lift case1 in
      let case_expr1 = compile_expression ~raise expr1 in
      let cond1 = e_constant ~loc:loc1 (Const C_EQ) [switch_expr; case_expr1] in
      let cond1 = (match fallthrough_conditions with
      ...
      let code1 = Option.map stmnts1 ~f:(fun s -> statement_result_to_expression @@ compile_statements ~raise s) in
      let part1 = Option.map code1 ~f:(fun code1 -> e_cond ~loc:loc1 cond1 code1 (e_unit ())) in

      let loc2 = Location.lift case2 in
      let case_expr2 = compile_expression ~raise expr2 in
      let cond2 = e_constant ~loc:loc2 (Const C_EQ) [switch_expr; case_expr2] in
      let cond2 = e_constant ~loc:loc2 (Const C_OR) [cond1; cond2] in
      let code2 = statement_result_to_expression @@ compile_statements ~case_return:true ~raise stmnts2 in
      let all_conditions = expr1::expr2::all_conditions in
      let rest = compile_cases cases None all_conditions in
      let part2 = e_cond ~loc:loc2 cond2 code2 rest in
      
      (match part1 with
      | Some part1 -> e_sequence part1 part2
      | None -> part2)
    ...
    | Case (case1, expr1, SwitchReturn stmnts1)::
      ...
      e_cond ~loc:loc1 cond1 code1 part2

    (* Case - Default *)
    | Case (kwd_case, expr, SwitchFallthrough switch_statements)::
      Default (_, DefaultBreak default_statements)::[] -> 
      ...
      e_sequence code rest
    
    | Case (kwd_case, expr, SwitchBreak switch_statements)::
      Default (_, DefaultBreak default_statements)::[] -> 
      let all_conditions = expr::all_conditions in
      let loc = Location.lift kwd_case in
      let case_expr = compile_expression ~raise expr in
      ...
      let default_statements = Option.map default_statements ~f:(fun s -> statement_result_to_expression @@
        compile_statements ~raise s) in
      let all_cond = or_conditions switch_expr all_conditions in
      let all_cond = e_constant (Const C_NOT) [all_cond] in
      let default_statements = Option.map default_statements ~f:(fun default_statements -> 
        e_cond all_cond default_statements (e_unit ())) in
      let default_statements = (match default_statements with
      | Some default_statements -> default_statements 
      | None -> e_unit ()) in
    ...
    | Case (kwd_case, expr, SwitchReturn switch_statements)::
      Default (_, DefaultBreak default_statements)::[] -> 
      let all_conditions = expr::all_conditions in
      let loc = Location.lift kwd_case in
      let case_expr = compile_expression ~raise expr in
      ...
      let rest = rest_of_the_code () in
      let else_clause = (match default_statements with
      | Some default_statements -> e_sequence default_statements rest
      | None -> rest) in
      
      e_cond ~loc condition then_clause else_clause

    | Case (kwd_case, expr, SwitchFallthrough switch_statements)::
      Default (_, DefaultReturn default_statements)::[] -> 
      let all_conditions = expr::all_conditions in
      let loc = Location.lift kwd_case in
      let case_expr = compile_expression ~raise expr in
      let condition = e_constant ~loc (Const C_EQ) [switch_expr; case_expr] in
      ...
      | None -> condition) in
      let all_cond = or_conditions switch_expr all_conditions in
      let not_all_cond = e_constant (Const C_NOT) [all_cond] in
      let cond = e_constant (Const C_OR) [not_all_cond; condition] in

      let default_statements = statement_result_to_expression @@ compile_statements ~raise default_statements in

      (match switch_statements with
      | Some switch_statements -> 
        let switch_statements = statement_result_to_expression @@ compile_statements ~case_return:true ~raise switch_statements in
        let rest = rest_of_the_code () in
        let then_clause = e_sequence (e_cond condition switch_statements (e_unit ())) default_statements in
        e_cond cond then_clause rest
    ...
    | Case (kwd_case, expr, SwitchBreak switch_statements)::
      ...
      let default_statements = statement_result_to_expression @@ compile_statements ~case_return:true ~raise default_statements in

      let all_cond = or_conditions switch_expr all_conditions in
      let not_all_cond = e_constant (Const C_NOT) [all_cond] in

      let else_clause = (e_sequence (e_cond ~loc condition (then_clause) (e_unit ())) rest) in

      e_cond not_all_cond default_statements else_clause
    ...
    | Case (kwd_case, expr, SwitchReturn switch_statements)::
      ...
      let then_clause = statement_result_to_expression @@ compile_statements ~case_return:true ~raise switch_statements in
      let default_statements = statement_result_to_expression @@ compile_statements ~case_return:true ~raise default_statements in
      ...
      let else_clause = e_cond not_all_cond default_statements rest in
...
and compile_statements ?(wrap=false) ?(case=false) ?(case_return=false) ~raise : CST.statements -> (CST.semi * CST.statement) list-> statement_result 
= fun statements rest ->
  let aux ?(case=false) result rest = function
    (_, hd) :: tl ->
      let wrapper = CST.SBlock {
        value = {
          inside = (hd, tl);
      ...
      let block = compile_statement ~wrap:false ~case ~raise wrapper rest in
      merge_statement_results result block
  | [] -> result
  in
  ...
  match hd, snd_ with 
    CST.SCond {value = {ifnot = None; _}; region}, (other :: tl) -> 
      let init = compile_statement ~wrap:false ~raise hd (if List.length snd_ > 0 then snd_ else rest) in 
      (match init with 
        Return {expression_content = E_cond e; location} -> 
          let else_clause_hd = compile_statement ~wrap:false ~raise (snd other) (if List.length snd_ > 0 then snd_ else rest) in
          let else_clause = aux else_clause_hd (if List.length snd_ > 0 then snd_ else rest) tl in
  ...
  | CST.SSwitch s, _ -> compile_switch_cases ~raise s (if case && (not case_return) then snd_ else if List.length snd_ > 0 then snd_ else rest)
  | _, _ -> 
...
and compile_statement ?(wrap=false) ?(case=false) ~raise : CST.statement -> (CST.semi * CST.statement) list -> statement_result 
= fun statement rest ->
  ...
  | SBlock {value = {inside; _}; region} when wrap = false ->
    ...
    let statements = self_statements ~wrap:true ~case inside rest in
    statements
  ...
  | SCond cond ->
    ...
    let then_clause  = self ~wrap:false cond.ifso rest in 
    let else_clause = Option.map ~f:(fun (_, s) -> self ~wrap:false s rest) cond.ifnot in
    let compile_clause = function 
  ...
  | SExport e ->
    ...
    compile_statement ~raise statement rest
  | SImport i ->
  ...
  | SForOf s ->
    ...
    let sr = compile_statement ~raise v.statement rest in
  ...
  | SWhile e ->
    ...
    let statement_result = compile_statement ~raise w.statement rest in
