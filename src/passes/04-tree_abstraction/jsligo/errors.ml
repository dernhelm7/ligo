...
type abs_error = [
  ...
  | `Concrete_jsligo_default_not_last_case of Region.t
...
let default_not_last_case d = `Concrete_jsligo_default_not_last_case d
...
let error_ppformat : display_format:string display_format ->
  ...
  | Human_readable | Dev -> (
    ...
    | `Concrete_jsligo_default_not_last_case d -> (
      Format.fprintf f "@[<hv>%a@.Default should be the last case of switch statement.@]"
          Snippet.pp_lift d
    )
...
let error_jsonformat : abs_error -> Yojson.Safe.t = fun a ->
  ...
  | `Concrete_jsligo_break_not_supported_ouside_case b ->
    ...
    json_error ~stage ~content
  | `Concrete_jsligo_default_not_last_case d -> 
    let message = `String "Default should be the last case of switch statement." in
    let loc = Format.asprintf "%a" Location.pp_lift d in
    let content = `Assoc [
      ("message", message);
      ("location", `String loc);] in
    json_error ~stage ~content
