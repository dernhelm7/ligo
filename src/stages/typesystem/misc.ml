  ...
  module Pattern = struct
    ...
    and s_expression_content : (T.expression_content,_) w = fun ~(substs : substs) -> function
      ...
      | T.E_lambda          { binder; output_type; result } ->
        ...
        T.E_lambda { binder; output_type; result }
      | T.E_let_in          { let_binder; rhs; let_result; attr = {inline; no_mutation ; view}} ->
    ...
    fun ~substs ->
      function
      | T.Declaration_constant {name ; binder ; expr ; attr={inline;no_mutation;view}} ->
        let binder = s_binder ~substs binder in
        let expr = s_expression ~substs expr in
        return @@ Declaration_constant {name; binder; expr; attr={inline;no_mutation;view}}
