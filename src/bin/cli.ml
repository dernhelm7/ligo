...
let compile_file =
  let f (entry_point, syntax, infer, protocol_version, display_format, disable_typecheck, michelson_format, output_file, warn, werror, michelson_comments) source_file () =
    return_result ~warn ?output_file @@ 
    Api.Compile.contract ~werror source_file entry_point syntax infer protocol_version display_format disable_typecheck michelson_format michelson_comments in
  ...
  Clic.command
    ...
    ~group:compile_group
    ~desc
    Clic.(args11 entry_point syntax infer protocol_version display_format disable_michelson_typechecking michelson_code_format output_file warn werror michelson_comments)
