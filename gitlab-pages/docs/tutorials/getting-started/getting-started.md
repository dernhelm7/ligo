...
As we can never underline enough the importance of test in the context of smart-contract. We will now test our contract three time on different level :
...
Then we gonna point the client on a tezos node
```zsh
...
tezos-client originate contract increment \
              ...
              --init 10 --burn-cap 0.1
```
Again, you will receive several message from the node and you should get the confirmation that the contract has been published.
