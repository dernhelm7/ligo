...
let vscode_syntax_highlighting: string -> string -> string -> string -> SyntaxHighlighting.Textmate.t -> string Term.ret = fun dir syntax_file language_file syntax textmate ->
...
let output: string -> string -> string -> _ Term.ret = fun vim_directory vscode_directory emacs_directory -> 
...
let generate_syntax_highlighting =
  ...
  Term.(ret (const output $ vim_directory $ vscode_directory $ emacs_directory)), Term.info "LigoSyntaxHighlighting" ~exits ~doc
