...
(* Build system *)
  ...
  let solve_graph : graph -> file_name -> (_ list,error) result =
  ...
  let add_deps_to_env asts_typed (_file_name, (_meta,_c_unit,deps)) =
    ...
    let env_with_deps = add_modules_in_env (M.AST.get_env ()) deps in
    env_with_deps
  ...
  let compile_separate : file_name -> _ =
    fun main_file_name ->
  ...
  let compile_combined : file_name -> _ =
    fun file_name ->
